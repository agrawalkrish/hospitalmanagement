...
const handleQueryError = (error, res) => {
...
app.get("/patient-details", (req, res) => {
  db.query("SELECT * FROM PatientDetails", (error, results) => {
    if (error) return handleQueryError(error, res);
    res.json(results);
  });
});
...
app.post("/update-appointment/:id", (req, res) => {
  ...
  const sql = "UPDATE appointments SET appointment_status = 'Completed' WHERE appointment_id = ?";
...
app.post("/add-inpatientRecords", (req, res) => {
  ...
  db.query(sql, [
    ...
    if (error) return handleQueryError(error, res);
    res.json({ message: "Inpatient record added successfully!" });
  });
});
...
app.post("/update-bill/:id", (req, res) => {
  ...
  const sql = "UPDATE billing SET payment_status = 'Paid' WHERE bill_id = ?";
